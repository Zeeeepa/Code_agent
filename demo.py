#!/usr/bin/env python3
"""
CodeGen Integration Demo

This script demonstrates the integration of all three CodeGen components:
1. Issue Solver
2. Context Manager 
3. CI/CD Workflow
...
from codegen_config import get_config, init_config_from_args
...
def main():
    parser = argparse.ArgumentParser(description="CodeGen Integration Demo")
    ...
    print("=" * 80)
    print("CodeGen Integration Demo")
    print("=" * 80)
    
    # Step 1: Collect context using Context Manager
    print("\n[Step 1] Collecting context...")
    context_cmd = f"python codegen_runner.py --mode context collect --issue {args.issue} --output context_{args.issue}.json"
    ...
    issue_cmd = f"python codegen_runner.py --mode issue --issue-number {args.issue} --task-type bug"
    ...
    workflow_cmd = f"python codegen_runner.py --mode workflow --repo-name {args.repo}"
